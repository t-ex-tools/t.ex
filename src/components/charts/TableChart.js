...
export default {
  ...
  methods: {
    ...
    initialize: function() {
      ...
      */
      ...
      })).concat([new google.visualization.ControlWrapper({
      ...
      this.filter
        .slice(0, this.filter.length-1)
        .forEach((f) => {
          google.visualization.events.addListener(f, "statechange", (e) => {
            let x = f.getState();
            document.querySelector("#values-" + e.container.id).innerHTML = "[" + x.lowValue + ", " + x.highValue + "]";
          });
        });
    ...
    process(n) {
      ...
      this.data = new google.visualization.DataTable(Util.toJsLiteral(n, this.labels, this.totals));
  ...
  template: /*html*/`
    <div :id="this.id()" class="mt-3">
      <div>
        <b-button variant="outline-primary" class="float-right" @click="exportData">
